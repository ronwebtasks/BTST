...
if st.button("🔍 Fetch Live Contract Data"):
    ...
    try:
                # --- FIXED LIVE FEED LOOKUP BLOCK WITH CUSTOM HEADER ROTATION ---
        import requests
        import random
        
        # Explicit browser signatures to mask requests on Streamlit Cloud servers
        USER_AGENTS = [
        ...
        # Re-initialize custom pipeline session 
        ...
        # Inject the authorized session safely into the object instance
        ticker_obj = yf.Ticker(meta["symbol"], session=custom_session)
        
        # Fetch live spot price smoothly
